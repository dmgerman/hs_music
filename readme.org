#+TITLE: hs_music Spoon
#+AUTHOR: Daniel M German
#+VERSION: 0.3

A Hammerspoon spoon for controlling Apple Music playback and volume.

* Features

- *Playback Control*: Play/pause, next/previous track, skip to next album
- *Track Info*: Get and display current track name, artist, and album
- *Volume Control*: Set, get, and adjust Music app volume (0-100)
- *Album Library*: Curated album list file driving "play a specific album",
  "play a random album", and an =hs.chooser= picker
- *Hotkey Bindings*: Fully configurable hotkey support for all operations
- *Non-blocking*: Album skip uses async callbacks to not block Hammerspoon

** Playback Control

- =togglePlayPause()= - Play or pause current track
- =nextTrack()= - Skip to next track
- =previousTrack()= - Go to previous track
- =nextAlbum()= - Skip to the next album (asynchronous, non-blocking)
- =previousAlbum()= - Skip to the previous album (asynchronous, non-blocking)
- =play(=)          - Forces to start playing a track
- =stop()=          - Stops playing a track. Note that after stop, play will restart the playlist/album
  

** Track Information and play status

- =getCurrentTrack()= - Returns formatted track info (customizable via =trackFormat=)
- =getCurrentArtist()= - Returns current artist name
- =showCurrentTrack()= - Display track info in alert (uses =alertDuration= and =trackFormat=)
- =isMusicPlaying()=   - True is a track is playing

** Volume Control

- =setVolume(level)= - Set volume to a specific level (0-100 percent); returns new volume or nil
- =getVolume()= - Get current volume as percentage (0-100)
- =adjustVolume(delta)= - Adjust volume by a given amount (can be negative); returns new volume or nil

** Album Library

The spoon reads a curated list of albums from a text file and offers three
ways to play from it. All three drive Music's UI: focus the search field
with ⌘F, paste the query, then walk the accessibility tree to find the
first card in the "Albums" results section and trigger its play overlay.

- =playAlbum(band, album)= - Search Music for =band, album= and start playback
  of the first matching album
- =playRandomAlbum([path])= - Pick a random entry from the album list file
  and pass it to =playAlbum=
- =chooseAlbum([path])= - Show an =hs.chooser= of the album list (sorted by
  band, then album, case-insensitively) and play the selected entry
- =addCurrentAlbum([path])= - Append the currently playing album to the
  list as =Band|Album= (skips if a case-insensitive match already exists)

*** File format

Default path is =~/.hammerspoon/albums.txt= (configurable via
=albumListPath=). One entry per line, =Band|Album=:

#+BEGIN_SRC text
Radiohead|OK Computer
The Cure|Disintegration
# lines whose first character is # are ignored (comments)
Peter Gabriel|So
#+END_SRC

Blank lines, lines whose first character is =#=, and lines without a =|=
are silently skipped.

* Configuration

** Alert Duration
Customize how long track info alerts display:
#+BEGIN_SRC lua
music.alertDuration = 3  -- seconds
#+END_SRC

** Track Format
Customize how track information is displayed using placeholders:
- ={name}= - Track name
- ={artist}= - Artist name
- ={album}= - Album name

Examples:
#+BEGIN_SRC lua
-- Default format
music.trackFormat = "{name} - {artist} [{album}]"

-- Artist first
music.trackFormat = "{artist} - {name}"

-- Album only
music.trackFormat = "{album} / {name}"

-- Minimal format
music.trackFormat = "{name}"
#+END_SRC

** Album List Path

Configure the path read by =playRandomAlbum= and =chooseAlbum=:
#+BEGIN_SRC lua
-- Default
music.albumListPath = os.getenv("HOME") .. "/.hammerspoon/albums.txt"

-- Or point at a curated file in your project
music.albumListPath = "/Users/me/Documents/favorite-albums.txt"
#+END_SRC

** Album Skip Attempts
Configure the maximum number of track skips when using =nextAlbum()=:
#+BEGIN_SRC lua
-- Default is 20
music.maxAlbumSkipAttempts = 20

-- Set lower for faster timeout (useful if albums are small)
music.maxAlbumSkipAttempts = 10

-- Set higher for longer playlists or more patience
music.maxAlbumSkipAttempts = 50
#+END_SRC

** Hotkey Bindings

Configure hotkey bindings to control Music playback:
- =togglePlayPause= - Hotkey for play/pause
- =nextTrack= - Hotkey for next track
- =previousTrack= - Hotkey for previous track
- =showTrack= - Hotkey to display current track info
- =nextAlbum= - Hotkey to skip to next album
- =previousAlbum= - Hotkey to skip to previous album
- =randomAlbum= - Hotkey for =playRandomAlbum()= (uses =albumListPath=)

Each hotkey requires =mods= (modifiers array) and =key= (key name):
#+BEGIN_SRC lua
music:init({
  togglePlayPause = { mods = {"cmd", "shift"}, key = "p" },
  nextTrack = { mods = {"cmd", "shift"}, key = "n" },
  previousTrack = { mods = {"cmd", "shift"}, key = "b" },
  showTrack = { mods = {"cmd", "shift"}, key = "i" },
  nextAlbum = { mods = {"cmd", "shift"}, key = "a" },
  previousAlbum = { mods = {"cmd", "shift"}, key = "z" },
  randomAlbum = { mods = {"cmd", "alt", "ctrl"}, key = "r" }
})
#+END_SRC

Available modifiers: =cmd=, =alt=, =shift=, =ctrl=

* Example Usage

** Complete Setup

#+BEGIN_SRC lua
local music = hs.loadSpoon("hs_music")

-- Configure display options
music.alertDuration = 3
music.trackFormat = "{artist} - {name}"

-- Initialize with hotkey bindings
music:init({
  togglePlayPause = { mods = {"cmd", "shift"}, key = "p" },
  nextTrack = { mods = {"cmd", "shift"}, key = "n" },
  previousTrack = { mods = {"cmd", "shift"}, key = "b" },
  showTrack = { mods = {"cmd", "shift"}, key = "i" },
  nextAlbum = { mods = {"cmd", "shift"}, key = "a" },
  previousAlbum = { mods = {"cmd", "shift"}, key = "z" }
})
#+END_SRC

** Manual Control

You can also call functions directly without hotkeys:

#+BEGIN_SRC lua
local music = hs.loadSpoon("hs_music")

-- Toggle play/pause
music:togglePlayPause()

-- Skip to next track
music:nextTrack()

-- Skip to previous track
music:previousTrack()

-- Skip to next album (asynchronous)
music:nextAlbum()

-- Skip to previous album (asynchronous)
music:previousAlbum()

-- Show current track with custom format
music:showCurrentTrack()

-- Set volume to specific level (50%)
music:setVolume(50)

-- Increase volume by 10%
music:adjustVolume(10)

-- Decrease volume by 5%
music:adjustVolume(-5)

-- Play a specific album by hand
music:playAlbum("Radiohead", "OK Computer")

-- Play a random album from ~/.hammerspoon/albums.txt
music:playRandomAlbum()

-- Show a chooser to pick an album from the list
music:chooseAlbum()

-- Add the currently playing album to ~/.hammerspoon/albums.txt
music:addCurrentAlbum()
#+END_SRC

* Requirements

- macOS with Apple Music app
- Hammerspoon

* Notes

- Uses native =hs.itunes= API for reliable playback and track information
- All operations verify Music app is running before executing
- Track info can be customized using =trackFormat= placeholders
- All operations gracefully handle errors with user alerts
